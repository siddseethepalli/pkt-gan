...
class WassersteinGAN(object):
    def __init__(self, g_net, d1_net, d2_net, d3_net, x_sampler, z_sampler, data, model):
        ...
        self.d1_ = self.d1_net(self.x_)
        self.d2_ = self.d2_net(self.x_)
        self.d3_ = self.d3_net(self.x_)

        self.g_loss = tf.reduce_mean(tf.add_n([self.d1_,self.d2_,self.d3_]))
        self.d1_loss = tf.reduce_mean(self.d) - tf.reduce_mean(self.d1_)
        self.d2_loss = tf.reduce_mean(self.d) - tf.reduce_mean(self.d2_)
        self.d3_loss = tf.reduce_mean(self.d) - tf.reduce_mean(self.d3_)
