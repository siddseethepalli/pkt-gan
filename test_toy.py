...
def build_generator(latent_size):
    ...
    # 10 classes in MNIST
    cls = Flatten()(Embedding(NUM_CLUSTERS, 100, embeddings_initializer="glorot_uniform")(image_class))
...
def train(starting_batch):
    ...
    for batch in range(starting_batch, nb_batches):
        ...
        image_batch, label_batch = gaussian_mixture_circle(batch_size,num_cluster=NUM_CLUSTERS)
        ...
        disc_loss = []
        for discriminator in discriminators:
            for _ in range(disc_train_ratio-1):
                discriminator.train_on_batch(X, [y, aux_y])
            disc_loss.append(discriminator.train_on_batch(X, [y, aux_y]))
        ...
        if batch % epoch_size == epoch_size - 1:
            ...
            def plot_scatter(fake, true, dir=None, filename="scatter",color="blue"):
                ...
                xx, yy = np.meshgrid(np.arange(-6, 6, h),
                     np.arange(-6, 6, h))
                for i in range(NUM_DISCRIMINATORS):
                    Z, _ = discriminators[i].predict(np.c_[xx.ravel(), yy.ravel()])
